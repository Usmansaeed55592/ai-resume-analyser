...
def build_regeneration_chain():
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system",
         ...
         "CRITICAL - DO NOT FABRICATE: The MISSING KEYWORDS list below is shown to you "
         "ONLY so you understand what the ATS scorer is checking for - it is NOT a "
         "checklist of things to add to the resume by default, and its mere presence "
         "is never a reason to add anything. You are allowed to add a missing keyword "
         "to the resume ONLY IF the ADDITIONAL NOTES text below explicitly states the "
         "candidate genuinely has that exact skill/tool/experience. If ADDITIONAL NOTES "
         "is '(none provided)' or empty, you MUST NOT add ANY of the missing keywords "
         "anywhere in the resume, in any form, under any heading. If ADDITIONAL NOTES "
         "mentions only some missing keywords, add ONLY those specific ones, using the "
         "SAME exact wording given in the MISSING KEYWORDS list (not a paraphrase), and "
         "leave every other missing keyword out completely. Never invent a new section "
         "(e.g. 'Additional Technical Experience', 'Other Skills', 'Extra Qualifications') "
         "to list unconfirmed missing keywords - if you cannot place a confirmed addition "
         "naturally into an existing section, do not add it at all. When in doubt about "
         "whether something is confirmed, leave it out: an honest resume with a lower "
         "score is always the correct output, never a fabricated one with a higher score.\n\n"
         ...
         "Never fabricate quantified results (e.g. 'improved X by 40%') unless that "
         "number already exists in the source material.\n\n"
...
def build_paragraph_regeneration_chain():
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system",
         ...
         "CRITICAL - DO NOT FABRICATE: The MISSING KEYWORDS list below is shown to you "
         "ONLY so you understand what the ATS scorer is checking for - it is NOT a "
         "checklist of things to add by default, and its mere presence is never a reason "
         "to add anything. You are allowed to add a missing keyword to a paragraph ONLY IF "
         "the ADDITIONAL NOTES text below explicitly states the candidate genuinely has "
         "that exact skill/tool/experience. If ADDITIONAL NOTES is '(none provided)' or "
         "empty, you MUST NOT add ANY of the missing keywords to ANY paragraph, in any "
         "form. If ADDITIONAL NOTES mentions only some missing keywords, add ONLY those "
         "specific ones - using the SAME exact wording given in the MISSING KEYWORDS list, "
         "not a paraphrase - into the single most relevant existing paragraph (e.g. a "
         "skills-list paragraph), and leave every other missing keyword out completely. "
         "If no existing paragraph is a natural fit for a confirmed addition, leave that "
         "keyword out rather than forcing it into an unrelated paragraph. Remember you "
         "cannot create a new paragraph either way. When in doubt about whether something "
         "is confirmed, leave it out: an honest resume with a lower score is always the "
         "correct output, never a fabricated one with a higher score.\n\n"
